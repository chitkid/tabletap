...
- A demo reset deletes guest sessions, so a guest mid-basket gets a 401 on the next read and lands on `/session-ended`. The basket survives in `localStorage` and is repriced against the new menu on the next claim — the ids are re-seeded, so those lines simply read "No longer on the menu".
