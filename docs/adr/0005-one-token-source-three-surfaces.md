...
`pnpm tokens` runs `scripts/generate-tokens.cjs` and emits `packages/ui/tokens.css`, the only stylesheet the apps consume. The generated file is committed, and CI regenerates it and fails on a diff, so the CSS can never drift from the JSON.
...
- The brand document stays upstream of the tokens: `scripts/sync-brand-to-tokens.cjs` reads the Quick Reference table and the colour sections of `docs/brand-guidelines.md` and regenerates the primitive scales. If the design system and the brand guidelines disagree, the design system is wrong.
...
- Generating shades mechanically has rough edges — the scales above 600 collapse toward black for the near-black `ink` and `olive` bases, and the brand sync re-creates the `primary`/`secondary`/`accent` keys on every run. Both are noted in `docs/backlog.md`; neither affects a value in use.
