...
Date: 2026-09-03
Status: accepted — amended 2026-09-14 (Russian localisation, Task 8): the decision stands, the mechanism that chooses a dish's `kind` does not. See **Decision**.
...
`kind` is one of `flatbread`, `bowl`, `side` or `drink`, each with its own composition — a topped round for flatbreads, a rimmed bowl of wedges, scattered pieces for sides, a glass with a straw for drinks. So the four sections of the menu read as four different kinds of thing at a glance, while no two dishes inside a section look alike.

**A category declares its kind; nothing infers it.** The kind is a column, `menu_categories.plate_kind`, written by the seed and carried to the browser on `MenuCategoryDto` (`PLATE_KINDS` lives in `@tabletap/shared`). This is amended from the original decision, which read the kind off the category's _name_ — `kindFromCategory` matched `'Flatbreads'` against the substring `flatbread` and fell through to `side`. That worked only for as long as a restaurant named its sections in English: the Russian localisation renamed all four at once (`Из печи`, `Горячее`, `Салаты и закуски`, `Напитки`), which under the old rule would have dropped every dish in the menu onto the fallback shape — the entire visual layer of a surface that has no photographs, lost with no test failing, because category names live in seed data and nothing read them as pictures. Naming is display copy and a plate is a property of the food, so the two are no longer the same fact. The DTO field carries no default, so a response that omits it fails to serialise rather than quietly drawing one shape for everything.
...
- Renaming a dish redraws it. `Plate` takes an optional `seed` for the case where that matters — pass a stable id and the illustration survives the rename. Renaming a _category_ no longer redraws anything, which is the point of the amendment above.
- A category created through the admin editor takes the column's default (`DEFAULT_PLATE_KIND`, `side`). Choosing a plate shape per category is a product decision nobody has asked for yet; what the amendment removes is the shape being guessed from a word.
