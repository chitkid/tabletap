...
Applied: guest-surface "Add to cart" and quantity steppers meet the 44x44px minimum with 8px+ gaps (see `design-system/tabletap/MASTER.md` pre-delivery checklist).
...
Applied: empty basket and empty order-history states use brand voice ("Nothing in the basket yet.") per `docs/brand-guidelines.md` §4; form and payment errors are plain and actionable ("That email and password don't match.") with the error shown under the field, not apologetic and not only at the top.
...
Only one result was returned (the query matched a single row in `ux-guidelines.csv`, not zero). No row specifically covers `aria-live` region politeness levels for live data — defaults used for that part: order-status changes on the kitchen display and the guest's order-tracking screen use `aria-live="polite"` (per the Quick Reference `aria-live-errors` / `toast-accessibility` guidance bundled with the skill, which specifies `aria-live="polite"` for non-urgent async updates and toasts), and icon-only controls (e.g. a quantity stepper's plus/minus, the kitchen ticket's bump control) carry `aria-label` per the result above.
...
Only four results were returned for this query (search asked for up to 5). Applied: order placement and admin menu-item mutations use Server Actions rather than a hand-rolled API route per mutation, consistent with the App Router guidance above.
...
Applied: admin forms (menu item edit, settings) and any guest-surface dialogs (item detail, cart review) use shadcn's `Dialog` + `Form`/`FormField`/`FormMessage` pattern with Zod schemas, keeping error display and announcement consistent with the States section above.
