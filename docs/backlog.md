...
- A demo reset invalidates every previously signed table token. `seed --reset` deletes and re-inserts `tables`, so the ids change and a token minted before the reset resolves to a table that no longer exists — `POST /api/guest/claim` answers 404. It costs nothing today, because the landing signs a fresh token on every request and `db:seed` prints new URLs, but it contradicts the reasoning in ADR 0002 for a 365-day TTL ("printed stickers have to outlive a demo reset"). Either the reset should preserve table ids, or ADR 0002 should say that printed stickers and the resetting demo tenant are different situations.
