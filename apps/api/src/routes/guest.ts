...
import { GUEST_COOKIE, createGuestSession, expireGuestSession, guestCookieOptions } from '../lib/guest-sessions';
  ...
  app.withTypeProvider<ZodTypeProvider>().post(
    ...
    async (request, reply) => {
      ...
      const session = await app.db.transaction(async (tx) => {
        ...
        const created = await createGuestSession(tx, { tableId: table.id, ttlHours: app.config.GUEST_SESSION_TTL_HOURS, now });
        await recordAudit(tx, { actorType: 'guest', actorId: created.id, action: 'guest.claimed', entityType: 'table', entityId: table.id, payload: { tableNumber: table.number } });
        return created;
      });
