...
**How the live connection works.** The board asks `POST /api/socket-token` over the ordinary proxied REST path — where the session cookie is first-party — and gets back a 60-second JWT, typed `tt-socket` and signed with `SOCKET_TOKEN_SECRET`; a fresh one is minted before every connection attempt, reconnects included. The Socket.io handshake verifies that token, and the **server** puts the socket in its room from the token's principal — `kitchen` for staff, `table:<tableId>` for a guest — so no client can ask to listen to somebody else's table. On connect the client emits `subscribe` and receives the whole board as a snapshot, which is why a reconnect needs no reconciliation; between snapshots an event is ignored when its `updatedAt` is older than the copy the board holds. The reasoning is in [ADR 0008](docs/adr/0008-realtime-delivery.md).

**Known limitation.** When the board loses its connection the "Reconnecting… the board will catch up." banner takes about 45 seconds to appear, because engine.io's default heartbeat (25 s ping interval plus a 20 s timeout) is what notices. A kitchen screen should know within seconds; lowering the server heartbeat is on the list for the final review.
...
- `e2e/kitchen-live.spec.ts` — the milestone's definition of done, in two browser contexts at once. Context K signs in as kitchen and opens `/kitchen`; context G claims table 7 from the landing and places an order. The test reads that order's `placedAt` from the API and records the wall-clock moment its ticket becomes visible in K; the gap must be under 500 ms. K then bumps the ticket through **Start** and **Ready**, and G's order page must say "Order #42 is ready." without a reload. Finally K goes offline and back online: the reconnect banner appears, then clears, and the board is compared against `GET /api/orders?active=1`.

On the local Compose stack that latency measured **72–90 ms** against the 500 ms budget. The offline half of the test allows a full minute, because the banner is triggered by engine.io's heartbeat rather than by the browser's offline event — see the known limitation above.

## Lighthouse

`pnpm lighthouse` claims table 7 through the web origin and signs in as the demo kitchen account, then audits three pages on Lighthouse's mobile preset: `/` without a cookie, `/menu` as a real guest, and `/kitchen` as real staff. It prints a table, writes `docs/lighthouse-results.json`, and exits non-zero when accessibility falls below `--min-a11y` (95). Locally all three scored **100** for accessibility.
